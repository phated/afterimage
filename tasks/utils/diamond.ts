 ...
 *    We use a service called [4byte](https://www.4byte.directory) to store
...
export class DiamondChanges {
  ...
  public getFacetCuts(
    ...
    const facetCuts: IDiamondWritable.FacetCutStruct[] = [];
   ...
   * @returns The `cuts` for your Diamond
   */
  public getRemoveCuts(
    cuts: IDiamondWritable.FacetCutStruct[]
  ): IDiamondWritable.FacetCutStruct[] {
    if (!this.previous) {
      throw new Error(
    ...
    const removeCuts: IDiamondWritable.FacetCutStruct[] = [];
  ...
  private isDiamondSpecSelector(selector: string): boolean {
    ...
    const baseDiamondSignatures = this.getSignatures({
      interface: baseDiamondInterface,
    });
    return [
      ...baseDiamondSignatures.map((signature) =>
        baseDiamondInterface.getSighash(signature)
      ),
    ].includes(selector);
  }
